...
void Graph::DFS(int v) {
  ...
  // Mete el vertice de partida en el stack
  stack.push(v);
...
void Graph::topologicalSortUtility(int v, std::set<int> &visited, StackLinkedList<int> &stackResult) {
  // Marcar el vertice actual v como visitado
  visited.insert(v);
  // Obtener los vecinos del vertice v
  // Si estos no han sido visitados llamar recursivamente a topologicalSortUtility
  NodeLinkedList<std::pair<int, int>> *ptr = adjList[v].getHead();
  while (ptr != nullptr) {
    std::pair<int, int> par = ptr->data;
    int u = par.first; // nodo u es vecino de v
